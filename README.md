...
| 兼容策略 | CLI 保留 `kokoro-tts`，避免破坏旧脚本；服务品牌统一为 AngeVoice |
...
# 自动选择最快的 registry
cd AngeVoice/docker
bash select-registry.sh v2.4.0

cd gpu
sudo docker compose up -d
```

支持的 registry（按国内可用性排序）：
...
CI 自动推送到三个 registry。普通 `main` 提交只做构建验证；打 `v*` tag 或手动触发 `publish=true` 时才推送。
预期镜像名：
```text
...
- 测试覆盖更新，CI 已启用 Python 3.10/3.11
