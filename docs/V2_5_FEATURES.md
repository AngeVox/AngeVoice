...
v2.6.x 完成服务端模块化重构、中文规则补强、Studio Web UI 刷新、安全启动校验、多模型运行时、MOSS 流式优化和持久化部署补齐，并统一项目品牌为 AngeVoice。批量合成、管理接口、可选 MP3、WebSocket 取消能力保持兼容。
...
| MOSS 运行时优化 | 单 engine executor + runtime lock，质量优先 MOSS 分包流式，参考音频裁剪与 prompt code LRU 缓存，输出削峰/去脉冲/边缘淡化，降低 clone OOM、爆音和重复编码开销 |
...
| 安全启动校验 | 管理后台必须搭配 `ANGEVOICE_ADMIN_PASSWORD`；占位 API Key 会被拒绝 |
...
```text
春花秋月何时了 -> 句末“了”按 liǎo 处理（Kokoro 用“瞭”提示，MOSS 用“蓼”提示，避免读成 le 或 liào）
...
管理后台/接口默认关闭。开启时必须设置后台密码，公网部署建议同时设置 API Key：
