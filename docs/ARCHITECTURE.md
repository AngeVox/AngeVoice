...
浏览器录音由 `static/studio/recording.js` 独占 AudioContext 节点、媒体轨道、PCM chunk、WAV 编码、15 秒自动停止与释放顺序。入口模块只注入能力判断、DOM 控件和 `{key, params}` 状态回调；录音模块在导入期不访问 DOM，也不持有全局应用 state。启动中途失败、手动停止、自动停止和取消都会停止所有媒体轨道、断开节点并关闭 AudioContext，生成的 File 再通过明确回调交给 prompt-audio 生命周期。

参考音频试听由 `static/studio/reference-audio-preview.js` 独占请求取消、迟到响应隔离、WAV MIME 修正、Object URL 和 `<audio>` 媒体事件。上传参考与已保存 Voice Profile 使用同一控制器；切换来源会通过 `AbortController` 取消旧请求，且即使旧响应已进入任务队列也不能覆盖新来源。清除、替换与页面离开都会撤销其持有的 Object URL。入口模块只注入 API 请求、当前 engine/profile 身份与 `{key, params}` 回调，不复制资源状态。
...
MOSS 使用独立分段长度 `MOSS_SEGMENT_LENGTH`（默认 120），不再强制复用 Kokoro 的 `KOKORO_SEGMENT_LENGTH`。这样 Kokoro 可以保持自己的分段策略，而 MOSS 优先降低中英文混合尾部漂移、卡顿和失真。Admin 后台保存的运行时配置会写入 `ANGEVOICE_RUNTIME_CONFIG_FILE`，默认 `/app/config/runtime-config.json`，在环境变量之后加载。
