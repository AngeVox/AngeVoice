...
| `response_format` | string | `wav` | 可选 `wav`、`pcm`；启用 FFmpeg 后支持 `mp3`、`ogg_opus`/`telegram_voice`、`m4a` |
...
- 响应体为音频字节流
- `Content-Type` 为 `audio/wav`、`audio/pcm`、`audio/mpeg`、`audio/ogg` 或 `audio/mp4`，取决于 `response_format`
- 响应头包含 `X-Request-ID`
...
> `/v1/audio/speech` 仅支持 JSON 格式。如需 MOSS/ZipVoice 参考音频克隆上传，请使用 `/api/tts` 的 multipart 方式。

## 旧版 `/api/tts` 接口

`/api/tts` 支持 JSON、query-string 和表单数据三种格式。当需要上传 MOSS/ZipVoice 参考音频进行克隆时，需使用此接口（multipart 上传不在 OpenAI 兼容 JSON 格式范围内）。
...
| `response_format` | `format` | 输出格式：`wav`、`pcm`；启用 FFmpeg 后支持 `mp3`、`ogg_opus`/`telegram_voice`、`m4a` |
...
ZipVoice 可使用已保存 Voice Profile，也可通过 `/api/tts` multipart 临时上传参考音频与参考文本。`prompt_text` 必须是参考音频中真实朗读的自然语言文本；代码、URL、纯符号或日志会被拒绝。
...
Telegram 语音格式示例，需启用 FFmpeg 转码：

```bash
curl -X POST "$BASE_URL/api/tts" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -F model=zipvoice \
  -F text="这条语音将输出为 OGG Opus 格式。" \
  -F voice=voice_001 \
...
| MP3 | `mp3` | `audio/mpeg`；需启用 FFmpeg 转码且安装 ffmpeg |
| OGG Opus | `ogg_opus`、`ogg`、`opus`、`telegram_voice` | `audio/ogg`；适合 Telegram voice 场景尝试；需启用 FFmpeg 转码 |
| M4A/AAC | `m4a` | `audio/mp4`；需启用 FFmpeg 转码 |

启用方式：管理后台打开“启用 FFmpeg 转码”，或设置 `ANGEVOICE_FFMPEG_ENABLED=true`。旧 `KOKORO_MP3_ENABLED=true` 仍兼容视为启用转码。常用环境变量：

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `ANGEVOICE_FFMPEG_ENABLED` | `false` | 是否启用非流式 HTTP 转码 |
| `ANGEVOICE_FFMPEG_BINARY` | `ffmpeg` | ffmpeg 可执行文件名或路径 |
| `ANGEVOICE_FFMPEG_TIMEOUT_SECONDS` | `30` | 单次转码超时 |
| `ANGEVOICE_AUDIO_MP3_BITRATE` | `192k` | MP3 码率；兼容原 `KOKORO_MP3_BITRATE` |
| `ANGEVOICE_AUDIO_OPUS_BITRATE` | `32k` | OGG Opus 码率 |
| `ANGEVOICE_AUDIO_AAC_BITRATE` | `96k` | M4A/AAC 码率 |

可通过 `GET /v1/audio/formats` 查询当前可用格式、FFmpeg 是否启用以及容器中是否安装了 ffmpeg。WebSocket 流式暂不输出 OGG/MP3/M4A，仍使用 `pcm_s16le` 或 `wav` 分包。

转码相关结构化错误码：

| code | 含义 | 处理 |
|---|---|---|
| `FFMPEG_DISABLED` | 请求了转码格式，但未启用 FFmpeg 转码 | 在管理后台启用“启用 FFmpeg 转码”，或设置 `ANGEVOICE_FFMPEG_ENABLED=true` |
| `FFMPEG_UNAVAILABLE` | 已启用转码，但找不到可执行的 ffmpeg | 安装 ffmpeg，或设置 `ANGEVOICE_FFMPEG_BINARY` 到正确路径 |
| `FFMPEG_CONVERSION_FAILED` | ffmpeg 运行失败或缺少编码器 | 检查容器/系统 ffmpeg 是否支持 `libmp3lame`、`libopus`、`aac`，或改用 `wav` |
...
| `ANGEVOICE_WEBSOCKET_STREAM_IDLE_TIMEOUT_SECONDS` | `120` | WebSocket 等待下一帧音频的空闲窗口，避免 MOSS 长文本首帧或分段间隙被误断开 |
| `ANGEVOICE_ENGINE_PROCESS_STREAM_DRAIN_SECONDS` | `30` | 隔离 Worker 取消流式请求后的排空等待窗口 |
| `ANGEVOICE_ENGINE_PROCESS_STREAM_IDLE_TIMEOUT_SECONDS` | `120` | 隔离 Worker 流式等待下一帧的空闲窗口，避免慢帧被误判为卡死 |
...
| `ANGEVOICE_RESTART_AFTER_IDLE_UNLOAD_DELAY_SECONDS` | `3` | 卸载成功后等待多久再退出；等待期间若有新请求或 WebSocket 会取消本次退出 |
| `ANGEVOICE_RESTART_AFTER_IDLE_UNLOAD_COOLDOWN_SECONDS` | `1800` | 触发冷却时间，避免异常环境中频繁退出重启 |
...
5. 如启用 `ANGEVOICE_RESTART_AFTER_IDLE_UNLOAD=true`，系统只会在这次空闲卸载成功后、且无活跃请求和 WebSocket 连接时安排一次进程退出；退出由 Docker/服务管理器自动拉起，不会在单纯空闲但没有发生卸载时循环重启
...
> 注意：重新加载模型需要一定时间（通常几秒到十几秒），首次请求的延迟会略高。如果显存/内存充裕，可以设置 `ANGEVOICE_IDLE_TIMEOUT_SECONDS=0` 禁用空闲卸载。彻底清理功能默认关闭，只有在需要清理 CUDA/ONNX Runtime 底层残留且容器具备自动重启策略时才建议开启。
...
| `NO_SYNTHESIZABLE_TEXT` / `未检测到可合成的中文或英文文本` | 输入是 URL、代码、日志、JSON、纯符号，或清理后没有可进入 tokenizer 的文本 | 改成自然语言后重试；纯数字如 `3.5` 会自动转成自然读法 |
| `FFMPEG_DISABLED` | 请求了 `mp3`、`ogg_opus`/`telegram_voice` 或 `m4a`，但未启用 FFmpeg 转码 | 管理后台启用 FFmpeg 转码，或设置 `ANGEVOICE_FFMPEG_ENABLED=true` |
...
| `ANGEVOICE_WEBSOCKET_STREAM_IDLE_TIMEOUT_SECONDS` | `120` | WebSocket 长文本流式空闲等待窗口 |
| `ANGEVOICE_ENGINE_PROCESS_STREAM_DRAIN_SECONDS` | `30` | 隔离 Worker 取消后的排空等待窗口 |
| `ANGEVOICE_ENGINE_PROCESS_STREAM_IDLE_TIMEOUT_SECONDS` | `120` | 隔离 Worker 流式空闲等待窗口 |
...
- Tuning：MOSS 长文本、流式、静音压缩、crossfade、峰值保护、空闲卸载和“空闲后彻底清理”等参数；内置 NAS 稳定、长文本旁白、低延迟流式、克隆质量优先四个预设。
