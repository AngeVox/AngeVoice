...
### 🚑 紧急修复
- 修复 MOSS ONNX 自动下载兜底：当 `/app/models/MOSS-TTS-Nano-100M-ONNX` 目录存在但缺少 `browser_poc_manifest.json` 或真实模型资产时，不再直接交给官方 runtime 报 500，而是自动尝试从 `MOSS_MODELSCOPE_REPO` 下载补全。
