#!/usr/bin/env bash
# AngeVoice 一键安装与管理脚本
# 自动检测 Docker、Compose、GPU、镜像网络，并推荐 CPU/GPU 画像；legacy-gpu 作为兼容。
...
prepare_config() {
  ...
  if [[ "$NON_INTERACTIVE" != "true" ]]; then
    log "管理后台默认已启用，首次可用 admin / admin123 登录；公网暴露前必须修改。"
    read -r -p "是否在首次启动前预设自定义管理员账号密码？[y/N]: " ans
    if [[ "$ans" =~ ^[Yy]$ ]]; then
      read -r -p "后台用户名（支持中文）[admin]: " admin_user
      admin_user="${admin_user:-admin}"
      read -r -s -p "后台密码: " admin_pass; echo
      [[ -n "$admin_pass" ]] || fail "后台密码不能为空。"
      local local_admin_env="docker/angevoice.local.env"
      [[ -f "$local_admin_env" ]] || cp docker/angevoice.local.env.example "$local_admin_env"
      chmod 600 "$local_admin_env" 2>/dev/null || true
      set_env_value "$local_admin_env" KOKORO_ADMIN_ENABLED true
      set_env_value "$local_admin_env" ANGEVOICE_ADMIN_USERNAME "$admin_user"
      set_env_value "$local_admin_env" ANGEVOICE_ADMIN_PASSWORD "$admin_pass"
      log "自定义管理员凭据已写入本地覆盖文件 $local_admin_env（已忽略版本控制）；后台持久化改密后可删除其中密码行。"
    fi
  fi
}
...
print_api_key_hint() {
  local key_file="$INSTALL_DIR/credentials/.angevoice-api-key"
  if [[ -f "$key_file" ]]; then
  ...
  log "复制该 token 到 Studio 设置里的 Bearer Token；也可使用默认后台 admin / admin123 首次登录查看/轮换，公网暴露前请立即改密。"
}
  ...
  if [[ "$NON_INTERACTIVE" != "true" ]]; then
    ...
    if [[ -n "$gpu_name" ]]; then
      ...
      if is_legacy_gpu_candidate; then
        warn "默认推荐通用 gpu 画像；legacy-gpu 仅作为 gpu 无法启动/不稳定时的兼容模式。"
      fi
    else
      log "未检测到 NVIDIA GPU。"
    fi
