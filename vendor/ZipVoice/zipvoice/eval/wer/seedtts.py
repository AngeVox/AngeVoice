...
def main(test_list, wav_path, extension, model_path, decode_path, lang, device):
    ...
    params = []
    with open(test_list, encoding="utf-8") as test_file:
    ...
    subses = []
    word_nums = 0
    def write_decode(text: str) -> None:
        if decode_path:
            with open(decode_path, "a", encoding="utf-8") as handle:
                handle.write(text)

    if decode_path:
        # Ensure the output directory exists and truncate a previous result safely.
        decode_dir = os.path.dirname(decode_path)
        if decode_dir and not os.path.exists(decode_dir):
            os.makedirs(decode_dir)
        with open(decode_path, "w", encoding="utf-8"):
            pass
    for wav_path, text_ref in tqdm(params):
        ...
        if decode_path:
            write_decode(f"{wav_path}\t{wer}\t{truth}\t{hypo}\t{inse}\t{dele}\t{subs}\n")
    ...
    print("-" * 50)
    if decode_path:
        write_decode(f"SeedTTS WER: {wer_avg}%\n")
        write_decode(f"WER: {wer}%\n")
        write_decode(
            f"Errors: {inse} insertions, {dele} deletions, {subs} substitutions, "
            f"over {word_nums} reference words\n"
        )
