...
# TensorBoard is a training-only dependency.  ZipVoice ONNX inference imports
# AttributeDict/str2bool from this module and must not require tensorboard at runtime.
if TYPE_CHECKING:
...
class MetricsTracker(collections.defaultdict):
    ...
    def __add__(self, other: "MetricsTracker") -> "MetricsTracker":
        ...
        for k, v in other.items():
            # Aggregate only finite scalar metrics; keep the original value type for addition.
            finite = bool(torch.isfinite(v).all().item()) if isinstance(v, torch.Tensor) else math.isfinite(float(v))
            if finite:
                ans[k] = ans[k] + v
        return ans
